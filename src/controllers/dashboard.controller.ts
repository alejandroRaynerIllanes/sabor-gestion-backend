...
export const obtenerResumenDashboard = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const [statsHoy, mesasActivas, platosPopulares, categoriasPopulares, ordenesRecientes] = await Promise.all([
    ...
    const traducirEstado = (estadoBD: string) => {
      switch(estadoBD) {
        ...
        case 'ENTREGADO': return 'Completada';
        case 'CERRADO': return 'Completada'; // O 'Pagado', según decidas
    ...
    const totalVentasCategorias = categoriasPopulares.reduce((acc, cat) => acc + cat.totalPedidos, 0);
    const categoriasFormateadas = categoriasPopulares.map(cat => ({
      nombre: cat._id,
      pedidos: cat.totalPedidos,
      porcentaje: totalVentasCategorias > 0 
    ...
    res.status(200).json({
      ...
      ordenesRecientes: ordenesRecientes.map(o => ({
        id: String(o._id).substring(String(o._id).length - 3).toUpperCase(),
    ...
    res.status(500).json({ mensaje: 'Error al generar el resumen del Dashboard', error: err.message });
  }
};