    ...
    try {
        ...
        if (usuarioExistente) {
            if (usuarioExistente.ci === ci) {
                return res.status(400).json({ mensaje: 'Ya existe un usuario registrado con este Carnet de Identidad' });
            }
...
export const actualizarUsuario = async (req: Request, res: Response): Promise<any> => {
    try {
        ...
        if (email !== usuario.email || ci !== usuario.ci) {
            ...
            if (usuarioExistente) {
                if (usuarioExistente.ci === ci) return res.status(400).json({ mensaje: 'El CI ya está en uso por otro usuario' });
        ...
        const usuarioActualizado = await Usuario.findByIdAndUpdate(id, datosActualizados, { new: true }).select('-password');
